...
#else
TULA_ENUM(Format, int, ecsv, ascii, memdump);
#endif
...
template <> struct IO<Format::ascii> {
    ...
    dump(OStream &os_, const Eigen::EigenBase<Derived> &data,
        ...
        SPDLOG_TRACE("dump as ascii, nrows={}, ncols={}, usecols={} "
                     "delim=\"{}\" comment=\"{}\"",
                     nrows, ncols, usecols, delim, comment);
        // get the usecols
        if (!usecols.empty()) {
            for (auto i : usecols) {
                if ((i < -ncols) || (i >= ncols))
    ...
    parse(IStream &is, std::vector<std::string> *header = nullptr,
        ...
        if (!usecols.empty()) {
            ...
            ncols_use = tula::meta::size_cast<Index>(usecols.size());
            SPDLOG_TRACE("using {} cols out of {}", ncols_use, ncols);
        }
        using Eigen::Dynamic;
        using Eigen::Map;
...
template <> struct IO<Format::ecsv> {
    ...
    dump(OStream &os_, const Eigen::EigenBase<Derived> &data,
        ...
        */
        // dump header
        tula::ecsv::dump_header<decltype(os), Scalar>(os, colnames, std::move(meta));
        // write data
        IO<Format::ascii>::dump(os, data, colnames, usecols,
                                tula::ecsv::spec::ECSV_DELIM_CHAR, std::nullopt);
        os_ << os.str();
        return os_;
    }
    ...
    parse(IStream &is, std::vector<std::string> *colnames = nullptr,
        ...
        const auto &[colnames_, meta_] = [&]() {
            try {
                ...
                //     tula::ecsv::parse_header(is, &lines_read);
                auto hdr = tula::ecsv::ECSVHeader::read(is, &lines_read);
                decltype(auto) colnames_ = hdr.colnames();
                decltype(auto) dtypes =hdr.datatypes();
                decltype(auto) meta_ = hdr.meta();
                // parse the data section
                if (!tula::ecsv::check_uniform_dtype<Scalar>(dtypes)) {
                    throw ParseError(fmt::format(
                        "the current implementation does not support "
                        "non-uniform datatype in header {}",
                        dtypes));
                }
        ...
        while (std::getline(is, line)) {
            ...
            for (auto it = line.begin(); it != line.end(); ++it) {
                ...
                // If i is not a delim, then append it to strnum
                if (tula::ecsv::spec::ECSV_DELIM_CHAR != *it) {
                    strnum += *it;
                    // continue to next char unless this is the last one
                    if (it + 1 != line.end())
                        continue;
