...
#include "kids/timestream/model.h"
    ...
    if (config.has("fitreportfile")) {
        ...
        const auto pattern0 =
            fmt::format("{}{:d}_{:06d}_{:03d}_0001.+\\.txt",
                        ...
                        m("obsid"), m("subobsid"));
        const auto pattern1 = meta.get_str("cal_file");
...
kids::TimeStreamSolverResult kids::TimeStreamSolver::operator()(
    ...
    const kids::TimeStreamSolver::Config &config_) const {
    ...
    constexpr auto modelspec = kids::CalibrationModel::S21WithGainLinTrend;
    using Model = kids::CalibrationModelTraits<modelspec>;
    // make mdl param on axis 1
    auto mdl = tone_data.array().bottomRows(Model::parameter_count);
